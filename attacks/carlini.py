"""PyTorch 0.4 Carlini and Wagner L2 attack algorithm.
From: https://github.com/jeromerony/fast_adversarial

Based on the original tf implementation:
...
class CarliniWagnerL2:
    ...
    def _step(self, model: nn.Module, optimizer: optim.Optimizer, inputs: torch.Tensor, tinputs: torch.Tensor,
              modifier: torch.Tensor, labels_onehot: torch.Tensor, targeted: bool, const: torch.Tensor) -> Tuple[
        ...
        real = (labels_onehot * logits).sum(1)
        other = ((1 - labels_onehot) * logits - labels_onehot * 10000).max(1)[0]
        if targeted:
    ...
    def attack(self, model: nn.Module, inputs: torch.Tensor, labels: torch.Tensor,
        ...
        o_best_attack = torch.zeros_like(inputs)
        ...
        if self.quantize:
            ...
            o_best_attack[adv_found] = self._quantize(model, inputs, o_best_attack[adv_found], labels, targeted=targeted)
